...
def extract_review(pg):
    try:
        ...
        digit = "".join(filter(str.isdigit, review.text_content().strip()))
        return int(digit) if digit else None
    except (Error, TimeoutError, ValueError, AttributeError) as e:
        print(f"Error Playwright: {e}")
        return None
...
def characteristics(pg):
    ...
    for div_parent in block_div_chr:
        ...
        for div_child in div_parent.locator("xpath=./div//div").all():
            ...
            try:
                key_name = spans[0].text_content().strip()
                val_desc = "".join(spans[1].text_content().split())
            except (Error, TimeoutError) as e:
                print(f"Error Playwright: {e}")
                pass
